...
pub fn createWindow(
    ...
    // Create the window
    const cookie: xcb.xcb_void_cookie_t = xcb.xcb_create_window(
        connection,
    ...
    const wm_delete_reply = xcb.xcb_intern_atom_reply(
        connection,
        xcb.xcb_intern_atom(
            connection,
            0,
    ...
    const wm_protocols_reply = xcb.xcb_intern_atom_reply(
        connection,
        xcb.xcb_intern_atom(
            connection,
            0,
...
pub fn createWindowSurface(vki: InstanceDispatch, instance: vk.Instance, win: Window) !vk.SurfaceKHR {
    var surf = vki.createXcbSurfaceKHR(
        instance,
        &.{
            .flags=.{},
