...
const log = utils.log.Logger("device");
...
pub const Device = struct {
    ...
    pub fn init(
        ...
        if (self.graphics) |q| {
            indices[q.idx] += 1;
            log.info("graphics idx: {}", .{q.idx});
        }
        if (self.compute) |q| {
            indices[q.idx] += 1;
            log.info("compute idx: {}", .{q.idx});
        }
        if (self.transfer) |q| {
            indices[q.idx] += 1;
            log.info("transfer idx: {}", .{q.idx});
        }
        if (self.present) |q| {
            indices[q.idx] += 1;
            log.info("present idx: {}", .{q.idx});
        }
    ...
    fn selectPhysicalDevice(
        ...
        for (pdevs) |pdev| {
            ...
            log.info("looking at device: {s}", .{props.device_name});

            if (reqs.discrete and !(props.device_type == vk.PhysicalDeviceType.discrete_gpu)) {
                log.warn("device {s} does not meet requirement of discrete gpu", .{props.device_name});
                continue;
            }
            ...
            const has_required_ext = blk: {
                ...
                for (reqs.extensions) |ext| {
                    for (extv) |e| {
                        ...
                        log.warn("device {s} does not have required extension {s}", .{ props.device_name, ext });
                        break :blk false;
                    }
                }
            ...
            if (ret.graphics == null and reqs.graphics) {
                log.warn("device {s} does not meet requirement of graphics queue", .{props.device_name});
                continue;
            }
            if (ret.present == null and reqs.present) {
                log.warn("device {s} does not meet requirement of present queue", .{props.device_name});
                continue;
            }
            if (ret.compute == null and reqs.compute) {
                log.warn("device {s} does not meet requirement of compute queue", .{props.device_name});
                continue;
            }
            if (ret.transfer == null and reqs.transfer) {
                log.warn("device {s} does not meet requirement of transfer queue", .{props.device_name});
                continue;
            }
            ...
            if (format_count == 0) {
                log.warn("device {s} does not meet requirement of surface formats", .{props.device_name});
                continue;
            }

            if (present_mode_count == 0) {
                log.warn("device {s} does not meet requirement of surface present modes", .{props.device_name});
                continue;
            }
            ...
            log.info("device {s} meets requirements", .{props.device_name});
            return ret;
        }
